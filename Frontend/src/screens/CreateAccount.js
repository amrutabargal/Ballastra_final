...
// 🔗 Backend base URL (IP/PORT apne hisaab se change kar sakti ho)
const BASE_URL =
  Platform.OS === "android"
    ? "http://10.0.2.2:5000/api" // Android emulator
    : "http://localhost:5000/api"; // iOS simulator
...
export default function RegisterScreen({ navigation }) {
  ...
  const handleCreateAccount = async () => {
    ...
    // basic email + password validation (optional)
    if (!email.includes("@")) {
    ...
    try {
      ...
      const body = {
        name: `${firstName} ${lastName}`, // backend ko name chahiye
        email,
        password,
        avatar_url: null, // future me yaha image URL de sakti ho
      };
      ...
      Alert.alert("Success", "Account created successfully!");
      // yaha tum Login ya Home me le ja sakti ho
      navigation.replace("Home"); // ya navigation.navigate("Login");
    } catch (error) {
      console.error("Signup Error:", error);
      Alert.alert("Error", "Unable to connect to server. Please try again.");
    } finally {
  ...
  const handleChangeAvatar = () => {
    console.log("Change avatar");
    // TODO: yaha image picker add kar sakti ho
  };
